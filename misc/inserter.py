                ...
                for j in range(len(sizes)):
                    try:
                        if i.get('origName') == sizes[j] and json1.get('data').get('products')[u].get('id') == articles[j]:
                            ...
                            if i.get('stocks') != []:
                                for w in tmp:
                                    info.append([w.get('wh'), currentBarcode, w.get('qty'), currentArticle, currentSize, company])
                            else:
                                for wh_name_in_arr in wh_names_arr:
                                    info.append([wh_name_in_arr[1], currentBarcode, 0, currentArticle, currentSize, company])
                    except:
                        pass
        return list(map(list, {tuple(x) for x in info}))
    except:
        return []
# data_actual_info_cards = cards_info_data()
# print(inserter())